            ...
            <div class="card bg-secondary border-0">
              <div class="card-body px-lg-5 py-lg-5">
                <div class="text-center text-muted mb-4">
                  <h1>{{ $t('Créer un nouveau compte') }}</h1>
                </div>
                <form role="form" @submit.prevent="onSubmit">
                  ...
                  <form-group>
                    ...
                    <input slot="input" class="form-control" :placeholder="`${$t('SIREN')}`" type="text" v-model="siren">
                  </form-group>
                  <form-group>
                    <i slot="icon" class="fas fa-money-check-alt"></i>
                    <input slot="input" class="form-control" :placeholder="`${$t('TVA')}`" type="text" v-model="tva">
                  </form-group>
                  <form-group>
                    ...
                    <div class="col-12">
                      <div class="custom-control custom-control-alternative custom-checkbox">
                        ...
                        <label class="custom-control-label" for="customCheckRegister">
                          <i18n path="J'accèpte la politique" class="text-muted">
                            <a href="https://my-sms.pro/conditions-generales">{{ $t('de Confidentialité') }}</a>
                          </i18n>
                        </label>
                      </div>
                    </div>
                  ...
                  <p v-if="hasError" class="text-danger text-sm">{{ $t(errorMessage) }}</p>

                  <div class="text-center">
                    <button type="submit" :disabled="!privacy" class="btn btn-primary mt-4">{{ $t('Créer le compte') }}</button>
                  </div>
                </form>
              </div>
            </div>
    ...
    onSubmit() {
      ...
      this.register({ company, siren, tva, firstname, lastname, userRole, street, city, postcode, telephone, mobile, email, password })
        .then(() => {
          ...
          this.getUser().then(() => this.$router.push({ name: 'dashboard' }));
        })
        .catch(({ data }) => {
